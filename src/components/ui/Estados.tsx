...
export function Vazio({
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center gap-2 py-10 px-4">
      ...
      <p className="text-sm text-tx-1">{titulo}</p>
      {descricao && <p className="text-[13px] text-tx-2 max-w-xs">{descricao}</p>}
...
export function Skeleton({ className = '' }: { className?: string }) {
  return <div className={`animate-pulse rounded-[6px] bg-sf-2 ${className}`} />
}
...
export function Erro({ mensagem, onTentar }: { mensagem: string; onTentar?: () => void }) {
  return (
    <div className="flex flex-col items-center gap-2 py-10 px-4 text-center">
      <p className="text-sm text-err">{mensagem}</p>
      {onTentar && (
