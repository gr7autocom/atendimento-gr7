    ...
    <div className="flex flex-col gap-3">
      {podeGerenciar && (
        <div className="relative">
          <div className="flex items-center gap-2 h-9 px-2.5 rounded-2 bg-sf-2 border border-bd-campo focus-within:border-br-2">
            ...
            <input
              ...
              className="flex-1 min-w-0 bg-transparent text-corpo text-tx-1 placeholder:text-tx-3 focus:outline-none"
            />
            <Plus size={15} className="text-tx-3 shrink-0" />
          </div>
          {termo && (
            <div className="absolute z-10 mt-1 w-full rounded-2 border border-bd-campo bg-sf-1 shadow-lg max-h-52 overflow-y-auto">
              {candidatos.length === 0 ? (
                <p className="px-3 py-2.5 text-apoio text-tx-3">Nenhum atendente encontrado.</p>
              ) : (
                candidatos.map((u) => (
                  <button
                    key={u.id}
                    ...
                    className="w-full flex items-center gap-2.5 px-2.5 py-2 text-left hover:bg-sf-2 transicao"
                  >
                    <Avatar nome={u.nome} fotoUrl={u.foto_url} tamanho={26} />
                    <span className="text-corpo text-tx-1 truncate">{u.nome}</span>
                  </button>
                ))
              )}
            </div>
      ...
      <div className="flex flex-col gap-2.5">
        {responsavel && (
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0 flex-1">
              <div className="text-corpo text-tx-1 truncate">{responsavel.nome}</div>
              <div className="text-mini text-br-2">Responsável</div>
            </div>
          </div>
        )}
        ...
        {participantes.map((p) => (
          <div key={p.id} className="flex items-center gap-2.5">
            ...
            <div className="min-w-0 flex-1">
              <div className="text-corpo text-tx-1 truncate">{p.usuario?.nome ?? 'Atendente'}</div>
              <div className="text-mini text-tx-3">Participante</div>
            </div>
            {podeGerenciar && (
              <button
                type="button"
                ...
                className="shrink-0 w-7 h-7 grid place-items-center rounded-1 text-tx-3 hover:text-err hover:bg-err-soft transicao"
              >
                <X size={15} />
              </button>
            )}
        ...
        {!responsavel && participantes.length === 0 && (
          <p className="text-apoio text-tx-3">Ninguém no atendimento ainda.</p>
        )}
      </div>
    </div>
  )
