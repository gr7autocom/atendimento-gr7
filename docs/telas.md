...
| **Departamentos** | grid de cards (número/nome/ativo + selo **Disponível/Fora de horário**). Clicar no card abre a **tela dedicada** `/admin/departamentos/:id` com blocos **Dados**, **Horário de atendimento** (7 colunas por dia; vazio = comercial) e **Motivos de finalização**. O **modal** fica só para **criar** um departamento (número + nome) | sim |
