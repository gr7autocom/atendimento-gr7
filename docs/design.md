...
- **Estado como ponto colorido:** `Ponto` ([`ui/Selo.tsx`](../src/components/ui/Selo.tsx)), com a cor por parâmetro e o rótulo ao lado. `PontoStatus` (status do atendimento) é o consumidor. A cor é sempre **reforço**: o texto carrega a informação, senão quem não distingue as cores fica sem ela.
...
- **Anexo na conversa:** `ListaAnexos` ([`ui/Anexo.tsx`](../src/components/ui/Anexo.tsx)) — imagem aberta e clicável, áudio no player nativo, o resto como linha de arquivo com ícone por tipo. O visualizador usa o `Modal` com `largura="ampla"`, que foi acrescentada ao componente em vez de virar overlay próprio na tela.
- **Gravar áudio:** `GravadorAudio` e `BotaoGravar` ([`ui/GravadorAudio.tsx`](../src/components/ui/GravadorAudio.tsx)), portados do Talks. O áudio **não** sai no soltar do botão: para, vira arquivo e a pessoa escuta antes de decidir. As mensagens de erro do microfone dizem onde clicar em cada sistema, porque "permissão negada" sozinho não tem saída.
- **Ações do botão direito:** `MenuContexto` ([`ui/MenuContexto.tsx`](../src/components/ui/MenuContexto.tsx)). Reusa o `useFecharFora` para clique fora e Esc, e acrescenta rolagem (em captura, porque a conversa rola num contêiner interno) e redimensionamento. Vai por portal no `body`, senão a bolha com `overflow` o recortaria.
- **Marca do produto:** `Marca` ([`ui/Marca.tsx`](../src/components/ui/Marca.tsx)) — logo, separador e a palavra "Atendimento", em três tamanhos por papel (`sm` rodapé, `md` barra e cabeçalho, `lg` abertura de tela). Antes disso, cada um dos quatro lugares montava o próprio quadrado com um raio e um tamanho de texto diferentes. Três detalhes que o componente carrega e que se perdem quando se remonta à mão: `alt="GR7 Autocom"` (com `alt` vazio o leitor de tela anuncia só "Atendimento"), `width`/`height` explícitos (sem eles a imagem chega depois do texto e empurra o layout) e o separador em `aria-hidden`, que é pontuação visual e não parte do nome.
