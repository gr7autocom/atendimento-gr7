...
| ~~Nome e foto do contato sem cadastro manual~~ | **Feito em 2026-08-11.** `WhatsAppDriver.buscarPerfil` (`driver-uazapi.ts`) chama `POST /chat/details` com `preview: true`, só na criação de contato novo (`whatsapp-webhook`). `wa_name` tem prioridade sobre `wa_contactName`/`name` — é o mesmo tipo de dado que `nome_whatsapp` já grava, nome de agenda salva é outra coisa. Foto em `contatos.foto_url` (migration `20260811010000`), exibida em `Avatar` (já suportava `fotoUrl`, usado pelos usuários do painel). Falha na busca não derruba o atendimento (contato nasce sem foto, não 500). **Não retroativo:** contato que já existia antes não ganha foto sozinho |
...
> **Integração real, não mais mock (desde 2026-08-10).** O adapter e as três Edge Functions existem em `supabase/functions/`. O **`whatsapp-webhook` roda o bot (Fase 1+2) e está deployado** no Supabase compartilhado — boas-vindas, menu, identificação do potencial + auto-vínculo por CNPJ, fila, `#sair` e avaliação ao finalizar (ver [bot.md](bot.md)). As chaves da uazapi (`UAZAPI_BASE_URL`/`UAZAPI_TOKEN`/`UAZAPI_ADMIN_TOKEN`) são secrets do Supabase, e `criarDriver()` já usa o driver real. **O "A CONFIRMAR" do envelope real foi fechado**: chamado #67 nasceu de tráfego real (número de fora, setor escolhido, `na_fila`), provando que `normalizarWebhook` do driver uazapi parseia certo. Endpoint protegido por `WEBHOOK_SECRET`, aceito por header OU por `?secret=` na URL — a uazapi só consegue mandar por URL, então `whatsapp-conexao` já embute o segredo ao registrar o webhook. Passos de deploy em [../supabase/functions/README.md](../supabase/functions/README.md).
...
- **Conexão:** `whatsapp-conexao` chama `/instance/connect` (QR/pairing) e `/instance/status` para a aba **Conexão** (`/admin/conexao`) — o token fica na Edge Function, nunca no frontend. Uma **pílula de status na barra do topo** (`BarraTopo` + `useStatusBot`) mostra "Bot conectado/desconectado" para admin e atendente, para o time perceber quando a sessão cai. **Ligado de verdade desde 2026-08-10** (`src/lib/useConexaoWhatsapp.ts`): status revalidado a cada 15s, QR/pairing code gerado ao vivo, webhook apontado no mesmo clique. As chaves (`UAZAPI_BASE_URL`/`UAZAPI_TOKEN`/`UAZAPI_ADMIN_TOKEN`) são secrets do Supabase; `criarDriver()` troca o mock pelo driver real sozinho quando elas existem.
